...
_PROMPTS: dict[str, str] = {
    ...
    "seatbelt": (
        ...
        "Answer YES (violation, not wearing) if a meaningful portion of the chest, shoulder, "
        "or lap area is visible and no strap, webbing, or buckle matching that description "
        "appears anywhere in it. "
        "Answer NO (belt is worn) if ANY part of a strap or buckle is visible anywhere — even "
        "faint, partial, or at an unusual angle. "
        "Only answer NO purely for image-quality reasons if almost none of the chest, "
        "shoulder, or lap area is visible at all (e.g. a tight face-only close-up) — say so "
        "explicitly in the reason when that's the case.\n\n"
        "First write down exactly what you observe, then decide based on that.\n\n"
        'Answer ONLY with this exact JSON (no markdown, no extra text), in this field order:\n'
        '{"reason": "one concise sentence describing exactly what you observe", "verified": true, "confidence": 0.95}\n'
        'or\n'
